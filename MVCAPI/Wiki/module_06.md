...
We create a generic filter `ValidationFilter<T>` that intercepts requests and validates the body before the Controller logic flows.

```csharp
// Controller Registration
group.MapPost("/", ...)
     .AddControllerFilter<ValidationFilter<PostNewOrUpdatedDto>>();
```
...
Your API includes specific Controllers to simulate failures and verify your global error handler is working.

**Test Controllers** (`/api/test/`):
